...
void UTP_WeaponComponent::Fire()
{
	...
	if (ProjectileClass != nullptr)
	{
		...
		if (World != nullptr)
		{
			...
			if(Projectile)
			{
				const float NewDamage = Character->GetDamage();
	...
	--CurrentNumBullets;
	if(OnCurrentNumBulletsChanged.IsBound())
	{
		OnCurrentNumBulletsChanged.Execute(CurrentNumBullets);
	}
}
...
void UTP_WeaponComponent::CheckAimToEnemy()
{
	if(APlayerController* PlayerController = Cast<APlayerController>(Character->GetController()))
	{
		const APlayerCameraManager* PlayerCameraManager = PlayerController->PlayerCameraManager;
		const FVector StartLocation = PlayerCameraManager->GetCameraLocation() + PlayerCameraManager->GetActorForwardVector() * 100.f;
		const FVector EndLocation = PlayerCameraManager->GetCameraLocation() + PlayerCameraManager->GetActorForwardVector() * 9999.9f;
		FHitResult LineTraceHitResult;
		if(GetWorld()->LineTraceSingleByChannel(LineTraceHitResult, StartLocation, EndLocation, ECC_Visibility))
		{
			if(LineTraceHitResult.GetActor() != nullptr && LineTraceHitResult.GetActor()->IsA(AUTAD_UI_FPS_Enemy::StaticClass()))
			{
				if(OnAimToEnemy.IsBound())
				{
					OnAimToEnemy.Execute(true);
				}
			}
			else
			{

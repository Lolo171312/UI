...
void AUTAD_UI_FPSProjectile::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	if ((OtherActor != nullptr) && (OtherActor != this))
	{
		...
		{
			Character->SetHealth(Character->GetHealth() - Damage, (GetOwner() != nullptr)? GetOwner()->GetActorLocation() : GetActorLocation());
			Destroy();
			return;
		}
...
void AUTAD_UI_FPSProjectile::SetDamage(const int NewDamage)
{
	Damage = NewDamage;
